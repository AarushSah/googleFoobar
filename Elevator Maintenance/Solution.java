...
public class Solution {
    ...
    public static String[] solution(String[] l) {
        Arrays.sort(l, (s1, s2) -> {
            ...
            int maj = compare(s1versions, s2versions, 0);
            if (maj != 0) {
                return maj;
            }
            int min = compare(s1versions, s2versions, 1);
            if (min != 0) {
                return min;
            }
            int rev = compare(s1versions, s2versions, 2);
            if (rev != 0) {
                return rev;
            }
    ...
    public static int compare(String[] s1versions, String[] s2versions, int versionType) {
        int version1 = versionType >= s1versions.length ? 0 : Integer.parseInt(s1versions[versionType]);
        int version2 = versionType >= s2versions.length ? 0 : Integer.parseInt(s2versions[versionType]);
